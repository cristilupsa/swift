...
func calculeaza(coef:(a: Float, b: Float, c: Float), x1: inout (re: Float, in:Float), x2: inout (re: Float, im: Float))
{
let delta = coef.b * coef.b - 4 * coef.a * coef.c
if delta >= 0{
  x1_re = (-coef.b - sqrt(delta)) / (2 * coef.a)
  x2_re = (-coef.b + sqrt(delta)) / (2 * coef.a)
  x1_im = 0
  x2_im = 0
}
else{
